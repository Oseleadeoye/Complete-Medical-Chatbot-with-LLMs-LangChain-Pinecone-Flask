...
Relevant document chunks (top 3)
     │
     ▼
Qwen3.6 27B (Groq API)  +  System Prompt
     │
     ▼
...
4. Initialises the LangChain RAG chain (retriever + Qwen3.6 27B via Groq)
...
1. **Contextualize** — the chat history and new question are passed to Qwen3.6 27B, which reformulates the question as a standalone query (so follow-up questions work correctly)
2. **Retrieve** — the reformulated question is embedded and used to search Pinecone for the 3 most similar document chunks
3. **Answer** — Qwen3.6 27B receives the retrieved chunks as context and generates a concise answer (max 3 sentences)
