...
Relevant document chunks (top 3)
     │
     ▼
Llama 3.3 (Groq API)  +  System Prompt
     │
     ▼
...
4. Initialises the LangChain RAG chain (retriever + Llama 3.3 via Groq)
...
3. **Answer** — Llama 3.3 receives the retrieved chunks as context and generates a concise answer (max 3 sentences)
